    ...
    return (
      <div className="flex gap-0.5">
        {[1, 2, 3, 4, 5].map((i) => (
          <Star
            key={i}
            className={`w-4 h-4 ${i <= rating ? 'text-yellow-400 fill-yellow-400' : 'text-muted-foreground/60'}`}
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Reviews and ratings collected after calls."
      />
      ...
      {summary.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {summary.map((s) => (
            <div key={s.repId} className="bg-card rounded-xl shadow-sm border p-4">
              ...
              <p className="text-xs text-muted-foreground mt-1">{s.totalReviews} review{s.totalReviews !== 1 ? 's' : ''}</p>
            </div>
          ))}
        </div>
      )}
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/40 border-b">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-medium text-muted-foreground">Representative</th>
          ...
          <tbody>
            {feedback.length === 0 ? (
              ...
              feedback.map((fb) => (
                <tr key={fb.id} className="border-b last:border-b-0 hover:bg-muted/50">
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      className="text-muted-foreground/80 hover:text-destructive transition"
                      title="Delete"
                    >
