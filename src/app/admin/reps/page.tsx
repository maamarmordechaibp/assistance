  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Manage admins and reps with phone access."
        actions={
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-card rounded-2xl shadow-xl p-6 w-full max-w-md mx-4">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => { setShowCreate(false); setShowPassword(false); }} className="p-1 rounded hover:bg-muted">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3 py-2 pr-10 border border-border rounded-lg text-sm focus:ring-2 focus:ring-ring focus:border-ring outline-none"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-muted-foreground/80 hover:text-muted-foreground"
                  >
                ...
                <>
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-1">
                      Phone Extension <span className="text-muted-foreground/80">(optional)</span>
                    </label>
                    <input
                  ...
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-1">
                      Dial-out Phone <span className="text-muted-foreground/80">(E.164 format)</span>
                    </label>
                    <input
                      ...
                      className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-ring focus:border-ring outline-none"
                    />
                    <p className="text-xs text-muted-foreground mt-1">Calls will ring this number when assigned.</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-1">
                      SIP URI <span className="text-muted-foreground/80">(optional, overrides phone)</span>
                    </label>
                    <input
                      value={form.sipUri}
                      ...
                      className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-ring focus:border-ring outline-none"
                    />
                    <p className="text-xs text-muted-foreground mt-1">For deskphones or softphones (Zoiper, Linphone). Free audio — no PSTN charge.</p>
                  </div>
                </>
              )}
              ...
              <button
                ...
                disabled={creating}
                className="w-full py-2.5 bg-accent text-white rounded-lg font-medium text-sm hover:bg-accent/90 transition disabled:opacity-50"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-card rounded-2xl shadow-xl p-6 w-full max-w-md mx-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setEditTarget(null)} className="p-1 rounded hover:bg-muted">
                <X className="w-5 h-5" />
              </button>
            </div>
            <p className="text-sm text-muted-foreground mb-4">{editTarget.email}</p>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Full Name</label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Phone Extension</label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">
                  Dial-out Phone <span className="text-muted-foreground/80">(E.164)</span>
                </label>
                <input
                  ...
                  className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-ring focus:border-ring outline-none"
                />
                <p className="text-xs text-muted-foreground mt-1">Cell or landline to ring when a call is assigned.</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">
                  SIP URI <span className="text-muted-foreground/80">(optional, preferred)</span>
                </label>
                <input
                  ...
                  className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-ring focus:border-ring outline-none"
                />
                <p className="text-xs text-muted-foreground mt-1">For deskphone/softphone. Overrides the phone number if set.</p>
              </div>
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-card rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { setResetTarget(null); setNewPassword(''); }} className="p-1 rounded hover:bg-muted">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-border rounded-lg text-sm mb-4 focus:ring-2 focus:ring-ring focus:border-ring outline-none"
            />
            <button
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {admins.map((user) => (
            <div key={user.id} className="bg-card rounded-xl shadow-sm border p-5">
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t">
                ...
                <div className="flex gap-1">
                  <button
                    ...
                    className="p-1.5 rounded hover:bg-muted text-muted-foreground/80 hover:text-muted-foreground"
                  ...
                  <button
                    ...
                    className="p-1.5 rounded hover:bg-destructive/10 text-muted-foreground/80 hover:text-destructive"
                    title="Delete user"
                  >
